 ...
 * =====================================================================================================================
 */
volatile struct s_ogc_sw_lib_os_nrtps_event
event_array[ OGC_SW_LIB_OS_NRTPS__EVENT_ARRAY_SIZE ] =
    { E_OGC_SW_LIB_OS_NRTPS_EVENT__EMPTY, 0U, NULL, NULL };
    ...
    )
{
    ...
    for ( ; init_index < OGC_SW_LIB_OS_NRTPS__EVENT_ARRAY_SIZE; init_index++ )
    {
        event_array[ init_index ].status = E_OGC_SW_LIB_OS_NRTPS_EVENT__EMPTY;
...
enum e_ogc_sw_lib_os_nrtps_status
ogc_sw_lib_os_nrtps_set(
    ...
    )
{
    ...
    for ( ; set_index < OGC_SW_LIB_OS_NRTPS__EVENT_ARRAY_SIZE; set_index++ )
    {
        if ( E_OGC_SW_LIB_OS_NRTPS_EVENT__EMPTY == event_array[ set_index ].status )
        {
            event_array[ set_index ].status = E_OGC_SW_LIB_OS_NRTPS_EVENT__PENDING;
            ...
            if ( set_index < ( OGC_SW_LIB_OS_NRTPS__EVENT_ARRAY_SIZE - 1U ) )
            {
                return ( E_OGC_SW_LIB_OS_NRTPS_STATUS__OCCUPIED );
            }
            else
            {
                return ( E_OGC_SW_LIB_OS_NRTPS_STATUS__FULL );
            }
        }
    }
    return ( E_OGC_SW_LIB_OS_NRTPS_STATUS__FAILURE );
}

/*                                         CLEAR for those times a scheduled event needs to be removed from the array */
enum e_ogc_sw_lib_os_nrtps_status
ogc_sw_lib_os_nrtps_clear(
    void( *function )( void* ),
    void* arguments
    )
{
    ...
    for ( ; clear_index < OGC_SW_LIB_OS_NRTPS__EVENT_ARRAY_SIZE; clear_index++ )
    {
        if ( ( E_OGC_SW_LIB_OS_NRTPS_EVENT__PENDING == event_array[ clear_index ].status )
             && ( function == event_array[ clear_index ].callback_function )
             && ( arguments == event_array[ clear_index ].argument_pointer )
           )
        {
            event_array[ clear_index ].status = E_OGC_SW_LIB_OS_NRTPS_EVENT__EMPTY;
    ...
    if ( true == task_found )
    {
        return ( E_OGC_SW_LIB_OS_NRTPS_STATUS__EMPTY );
    }
    else
    {
        return ( E_OGC_SW_LIB_OS_NRTPS_STATUS__FAILURE );
    ...
    )
{
    ...
    while ( start_index < OGC_SW_LIB_OS_NRTPS__EVENT_ARRAY_SIZE )
    {
        /* TODO: add tracking to detect if there are no pending tasks remaining...  there should always be something */
        if ( ( E_OGC_SW_LIB_OS_NRTPS_EVENT__PENDING == event_array[ start_index ].status )
           ...
           )
        {
            ...
            event_array[ start_index ].status = E_OGC_SW_LIB_OS_NRTPS_EVENT__EMPTY;
        }
        start_index++;
        if ( OGC_SW_LIB_OS_NRTPS__EVENT_ARRAY_SIZE == start_index )
        {
            start_index = 0U;
        }
    }
...
bool run_unit_tests__ogc_sw_lib_os_nrtps( void )
{
    /*                                     alter the event array, run init and see if everything clears appropriately */
    uint8_t test_index = 0U;
    for ( ; test_index < OGC_SW_LIB_OS_NRTPS__EVENT_ARRAY_SIZE; test_index++ )
    {
        event_array[ test_index ].status = E_OGC_SW_LIB_OS_NRTPS_EVENT__PENDING;
    ...
    {
        if ( ( E_OGC_SW_LIB_OS_NRTPS_EVENT__EMPTY != event_array[ test_index ].status )
    ...
    for ( ; test_index < ( LIB_OS_NRTPS__EVENT_ARRAY_SIZE - 1U ); test_index++ )
    {
        ...
        if ( E_OGC_SW_LIB_OS_NRTPS_STATUS__OCCUPIED != ret_value )
        {
            return ( false );
        }
    }
    ret_value = lib_os_nrtps_set( ( uint64_t )test_index, test_function, ( void* )test_index );
    if ( E_OGC_SW_LIB_OS_NRTPS_STATUS__FULL != ret_value )
    {
        return ( false );
    }
    ...
    if ( E_OGC_SW_LIB_OS_NRTPS_STATUS__FAILURE != ret_value )
    {
        return ( false );
    }
    ...
    if ( E_OGC_SW_LIB_OS_NRTPS_STATUS__EMPTY != ret_value )
    {
        return ( false );
    }
